...
Reports status information about an application activity an end user performed. For example, an administrator runs a database search or endpoint search. Or the administrator runs a command line interface command (e.g., expand\_storage).
...
[SEDR shows no 4100 or 4102 events](http://knowledge.broadcom.com/external/article/197319)

4102: Antivirus Detection (Endpoint)

Reports when an antivirus was detected on an endpoint.

[Many 4102 events are recorded](https://knowledge.broadcom.com/external/article?legacyId=TECH256704)
...
Reports when an IP, URL, or Domain was detected that is in a Symantec-provided deny list or the
...
Reports when a file was detected that is in a Symantec-provided deny list or the
